...
| `wallet/NO_ETH_GAS_LABEL`               | `noEthGasLabelRule`              | wallet  | info     | Deprecated, default-excluded advice when ETH/Gwei fee text is co-located with file-level Arc evidence.                                 | `wallet-rules.test.ts`, `c12-no-eth-gas-label-advice.test.ts`               | ArcLens adoption probe pins seven opted-in records and zero default records.                                  | Arc-specific guidance / low confidence | Deprecated advice    | Retain only for explicit human review; do not claim rendered UI or Arc ownership of the matched line. |
...
- `wallet/NO_ETH_GAS_LABEL`: C12 default-excludes and deprecates the file-level heuristic because it cannot separate rendered Arc UI from valid calculations, documentation, contrasts, or multichain siblings. Revisit only with evidence for a bounded rendered-UI contract.
...
- `wallet/NO_ETH_GAS_LABEL`: C12 covers default exclusion, explicit levels, uncertain sibling ownership, and the pinned ArcLens adoption result; do not expand regex fixtures while the rendered-UI boundary remains unsupported.
