...
**Status:** Active roadmap during C12 publication
...
### C12 — Implementation candidate: NO_ETH_GAS_LABEL Advice-only migration
...
The candidate preserves 19 known rules, seven known wallet rules, and four
canonical rules while reducing default execution to 15 rules and the default
wallet subset to six. Focused tests, the full test suite, fixture validation,
built-CLI probes, the pinned ArcLens adoption probe, final independent review,
and the full repository gate pass. Publication, exact-head CI, and merge remain
before completion.
...
5. Publish the reviewed C12 `wallet/NO_ETH_GAS_LABEL` Advice-only migration,
   verify exact-head CI, and merge before further regex changes.
